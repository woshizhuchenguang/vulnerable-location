...
def generator_of_data(data, labels,linetokens, vpointers, data_add,linetokens_add,vpointers_add, batchsize, maxlen, vector_dim):

    iter_num = int(len(data) / batchsize)
    i = 0

    #len(data)和len(data1)等长,labels应该是一样的
    while iter_num:
        ...
        #
        batchdata_add = data_add[i:i + batchsize]
        batched_input_add = process_sequences_shape(batchdata_add, maxLen=maxlen, vector_dim=vector_dim)
        #batched_labels_add = labels_add[i:i + batchsize]
        batched_linetokens_add = linetokens_add[i:i + batchsize]
        batched_vpointers_add = vpointers_add[i:i + batchsize]
        batched_vulner_places_add = []
        '''
        for vp_add in range(len(batched_vpointers_add)):
            place_sequence_add = sample_place_sequence(maxlen, batched_vpointers_add[vp_add], batched_linetokens_add[vp_add])
            vulner_places_add = np.diag(place_sequence_add)
            batched_vulner_places_add.append(vulner_places_add)
        batched_vulner_places_add = np.array(batched_vulner_places_add)
        '''
        #yield ([batched_input, batched_vulner_places, batched_input_add,batched_vulner_places_add], batched_labels)
        yield ([batched_input,  batched_vulner_places,batched_input_add], batched_labels)
        i = i + batchsize
