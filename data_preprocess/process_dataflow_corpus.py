...
f = open("/home/zcg/VulDeeLocator/src/record/synthetic and academic dataset/testcases_train.pkl",'rb')  #get the testcase ids of train sets and test sets
testcases += pickle.load(f)  
f.close()

#f = open("record/synthetic and academic datasets/testcases_test.pkl",'rb')
f = open("/home/zcg/VulDeeLocator/src/record/synthetic and academic dataset/testcases_test.pkl",'rb')
testcases += pickle.load(f)
f.close()
...
def get_sentences(slicepath, labelpath, corpuspath):
    ...
    for folder in os.listdir(slicepath):  
        ...
        filepath = os.path.join(labelpath, "slice2flawline_NO.pkl")#sard
        f = open(filepath, 'rb')
        ...
        for testcase_1 in os.listdir(os.path.join(slicepath, folder)):
            ...
            for testcase_2 in os.listdir(os.path.join(slicepath, folder, testcase_1)):
                if not os.path.isdir(os.path.join(slicepath, folder, testcase_1, testcase_2)):
                    continue
                                    ...
                                    if os.path.getsize(filepath) > 1:
                                        ...
                                        for sentence in sentences:
                                            ...
                                            if linenum_index in vlinenumlists:
                                                slice_vlinenum.append(token_index)#行号转成对应的toekn序列
...
if __name__ == '__main__':
    ...
    SLICEPATH = '/home/zcg/VulDeeLocator/program_zcg/SARD/SARD/slice_new/'  # path of slices generated by synthetic and academic datasets
    LABELPATH = '/home/zcg/VulDeeLocator/program_zcg/SARD/SARD/label_new/'   #path of labels
    CORPUSPATH = '/home/zcg/VulDeeLocator/program_zcg/SARD/SARD/corpus_new/'      #path of corpus
